...
public class SubtractButtonStrategy implements ButtonStrategy {
    ...
    public JButton createSpecificButton(String text, JLabel mainLabel, JLabel additionalLabel, CalculatorService calculatorService) {
        ...
        btn.addActionListener(e -> {
            if (validator.isCurrentStateValid(text, additionalLabel.getText())) {
                ...
                calculatorService.setAction(OperationType.SUBTRACT);
                mainLabel.setText(CalculatorConstants.ONLY_ZERO);

                if (additionalLabel.getText().contains(CalculatorConstants.EQUAL_SIGN)) {
                    additionalLabel.setText(calculatorService.getFirstNumber().concat(OperationType.SUBTRACT.getOperationSign()));
                } else {
                    additionalLabel.setText(additionalLabel.getText().concat(OperationType.SUBTRACT.getOperationSign()));
                }
            }
        });
        return btn;
